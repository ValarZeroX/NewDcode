$(function () {
    ...
    function calJD(ye, mo, da, ho, mi) { // 実数体上
        ...
        JD += ((ho - 8) * 60.0 + mi) / 1440.0 + 1721088.5;
    ...
    function convertPlanetHouse0(pos, csp) {
        var cusp0 = 0.0;
        var cusp1 = 0.0;
        var ang0 = 0.0;
        var ang1 = 0.0;
    ...
    function cnvglyph(adeg) {
        adeg = mod360(adeg);
        var sgn = Math.floor(adeg / 30.0);
        var deg = Math.floor(adeg - sgn * 30.0);
        var min = Math.floor((adeg - Math.floor(adeg)) * 60.0);
        var array = {};
        array[sgn] = deg + '°' + min
        return array;
    }
    ...
    $('.location').change(function () {
        ...
        if (location === '其他') {
            ...
            $('.longitude').val(locationArray[0])
            $('.latitude').val(locationArray[1])
        }
    })
    $('.submit-date').on('click', function () {
        $('.aspect').empty()
        $('.planet-positions').empty()
        $('#paper').empty()
        ...
        var planetsString = ['Sun', 'Moon', 'Mercury', 'Venus', 'Mars', 'Jupiter', 'Saturn', 'Uranus', 'Neptune', 'Pluto', 'NNode', 'Lilith']
        var planets = {}
        for (let index = 1; index <= planetsString.length; index++) {
            planets[planetsString[index - 1]] = []
            planets[planetsString[index - 1]].push(planetPosition[index])
        }

        var cuspLongitudes = new Array()
        cuspLongitudes = calHouseCusp2(parseInt(data.Year), parseInt(data.Month), parseInt(data.Day), parseInt(data.Hour), parseInt(data.Minutes), parseFloat(data.Longitude), parseFloat(data.Latitude), 1)

        var cusps = []
        for (let index = 1; index < cuspLongitudes.length; index++) {
            cusps[index - 1] = cuspLongitudes[index]
        }

        var planetHouse = new Array()
        var planetHouseInfo = {}
        planetHouse = convertPlanetHouse(planetPosition, cuspLongitudes)
        $.each(planetsString, function (index, value) {
            planetHouseInfo[value] = planetHouse[index + 1]
        })
        var dataRadix = {
            "planets": planets,
            "cusps": cusps
        ...
        var chart = new astrology.Chart('paper', chartWidth, chartWidth, {
            MARGIN: 80, COLOR_ARIES: '#FF0000', COLOR_TAURUS: '#008000', COLOR_GEMINI: '#FFFF00', COLOR_CANCER: '#C0C0C0', COLOR_LEO: '#FFD700', COLOR_VIRGO: '#4A4300', COLOR_LIBRA: '#B0E0E6', COLOR_SCORPIO: '#FFFAFA', COLOR_SAGITTARIUS: '#800080', COLOR_CAPRICORN: '#A52A2A', COLOR_AQUARIUS: "#6495ED", COLOR_PISCES: "#90EE90", SYMBOL_SCALE: scale
        })

        chart.radix(dataRadix).addPointsOfInterest({ "As": [cusps[0]], "Ic": [cusps[3]], "Ds": [cusps[6]], "Mc": [cusps[9]] }).aspects()

        //星位、宮位
        var glyph = {}
        $.each(planets, function (index, value) {
            glyph[index] = cnvglyph(value)
        })
        ...
        $.each(glyph, function (index, value) {
            ...
            $.each(value, function (houses, angle) {
                planetsContent += '<td><img width="25" height="25" src="/images/constellation/planet/' + index + '_symbol.svg" alt="' + planetDict[index] + '"></td>' + '<td>' + planetDict[index] + '</td>' + '<td>' + angle + '</td>' + '<td><img width="25" height="25" src="/images/constellation/constellation/' + constellation[houses] + '.svg" alt="' + sgnname[houses] + '"></td>' + '<td>' + sgnname[houses] + '</td>' + '<td>' + planetHouseInfo[index] + '</td>'
            })
            planetsContent += '</tr>'
        })
        planetsContent += "</table>"
        $('.planet-positions').append(planetsContent)

        //四大尖軸
        var aspect = { "ASC": cusps[0], "IC": cusps[3], "DSC": cusps[6], "MC": cusps[9] }
        var aspectGlyph = {}
        $.each(aspect, function (index, value) {
            aspectGlyph[index] = cnvglyph(value)
        })
