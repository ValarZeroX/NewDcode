...
@section('og_meta')
    <meta property="og:title" content="{{ $data['name'] }} - {{ __('tarot.tarot_card_meanings') }}">
    <meta property="og:description" content="{{ Str::limit(strip_tags($data['upright_content']), 150) }}">
    <meta property="og:image" content="{{ asset('images/deck/' . $data['image_key'] . '.jpg') }}">
...
@section('main')
    <div class="container">
        ...
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item"><a
                        href="/language/{{ app()->getLocale() }}/tarot/showall">{{ __('tarot.tarot_card_meanings')}}</a>
                </li>
                <li class="breadcrumb-item active" aria-current="page">{{$data['name']}}
                    {{ __('tarot.tarot_card_meanings')}}</li>
            </ol>
        </nav>
                        ...
                        <div class="card-body">
                            <div class="tarot-showcard">
                                <img src="/images/deck/{{$data['image_key']}}.jpg" class="figure-img img-fluid showcard"
                                    alt="{{$data['name']}} - {{ __('tarot.tarot_card_meanings') }}"
                                    title="{{$data['name']}} - {{ __('tarot.tarot_card_meanings') }}" width="160"
                                    height="280">
                            </div>
        ...
        <div class="alert alert-info" role="alert">
            ...
            <a class="btn  btn-success" href="/language/{{ app()->getLocale() }}/tarot/showall"
                role="button">{{ __('tarot.go_to_tarot_readings')}}</a>
        </div>
        <div class="alert alert-info" role="alert">
